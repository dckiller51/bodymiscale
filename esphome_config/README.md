# ESPHome Configuration for Xiaomi Mi Scale with Data Persistence and Multi-User Management

This ESPHome project provides a robust configuration to integrate a Xiaomi Mi Scale into Home Assistant, emphasizing data persistence and multi-user management. It allows tracking weight, impedance (if your scale supports it), and the last measurement time for each user, even after an ESPHome restart.

## Features

* **Data Persistence:** Weight, impedance, and last measurement time values are stored and restored after an ESPHome restart.
* **Multi-User Management:** The configuration is designed to support up to 5 users. You can configure the desired number of users by uncommenting the corresponding sections.
* **Last Measurement Time Sensor:** A timestamp sensor is provided for each user, indicating the date and time of the last measurement.
* **Flexibility for Scales Without Impedance:** Users with scales that only measure weight can easily comment out or remove the impedance-related sections.
...
3. **YAML File Configuration:** Copy the provided YAML code into your ESPHome configuration file.
4. **User Configuration:**
    * Set the desired number of users by uncommenting the corresponding sections.
    * Configure user names, weight ranges, and other parameters in the `substitutions` section.
5. **Impedance Management:**
    * If your scale does not measure impedance, comment out or remove the `impedance` sections in the configuration.
6. **Configuration Upload:** Upload the configuration to your ESP32 device using ESPHome.
7. **Home Assistant Integration:** Sensors will be automatically discovered by Home Assistant. You can add them to your dashboards.
